...
resource "aws_iam_role" "stats_lambda_role" {
  name = "${var.stats_lambda_name}-role"
...
# Stats Lambda Policy — DescribeTable + Scan (read-only) + Logs
resource "aws_iam_policy" "stats_lambda_policy" {
  name        = "${var.stats_lambda_name}-policy"
  description = "Policy for Void Map Stats Lambda to describe and scan DynamoDB and log to CloudWatch"
...
resource "aws_iam_role_policy_attachment" "stats_lambda_logs" {
  role       = aws_iam_role.stats_lambda_role.name
  policy_arn = aws_iam_policy.stats_lambda_policy.arn
}
...
data "archive_file" "stats_lambda_zip" {
  type        = "zip"
  source_file = "${path.module}/../lambdas/stats/handler.py"
  output_path = "${path.module}/stats.zip"
}
...
resource "aws_apigatewayv2_route" "options_stats_route" {
  api_id    = aws_apigatewayv2_api.http_api.id
  route_key = "OPTIONS /stats"
  target    = "integrations/${aws_apigatewayv2_integration.stats_integration.id}"
}
